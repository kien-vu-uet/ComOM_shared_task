...
def main():
    ...
    for file in input_files:
        ...
        segment_syllable_mapping = data_gene.segment_syllable_mapping
        # for segment_syllable_mapping_list in segment_syllable_mapping:
        #     print(segment_syllable_mapping_list)
        ...
        for index in range(len(standard_token_2_bert_idx)):
            ...
            segment_syllable_mapping_list = segment_syllable_mapping[index]
            ...
            # print([(i, tok) for i, tok in enumerate(list_token)])
            tmp_list_token = []
            for tok in list_token:
                tmp_list_token += tok.split("_")
            result += raw_sent + "\t" + " ".join(tmp_list_token) + "\n"
                        ...
                        if s_index != -1 and e_index != -1:
                            for i, l in idx_map.items():
                                ...
                                if e_index in l:
                                    new_e_index = i
                            # print(list_token)
                            # print(new_s_index, new_e_index)

                            # pair_dict[elem_tags[elem]] = [f"{i+1}&&{list_token[i]}" for i in range(new_s_index, new_e_index)]
                            pair_dict[elem_tags[elem]] = []
                            for i in range(new_s_index, new_e_index):
                                segment_key = f"{i+1}&&{list_token[i]}"
                                syllable_list = segment_syllable_mapping_list[segment_key]
                                pair_dict[elem_tags[elem]] += syllable_list
                        else:
                            pair_dict[elem_tags[elem]] = []
    ...
    zip_name = 'mode_multitask_with_softmax_before_crf_and_vncorenlp.zip'
