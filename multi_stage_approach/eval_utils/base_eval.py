...
class BaseEvaluation(object):
    ...
    def elem_dict_to_string(self, token_list, data_dict, symbol="_"):
        """
        :param token_list:
        :param data_dict: {elem: [(s_index, e_index)]}
        :return:
        ...
        for index, elem in enumerate(cur_elem_col):
            ...
            for elem_index, elem_representation in enumerate(data_dict[elem]):
                ...
                if e_index >= len(token_list):
                    e_index = len(token_list) - 1
                    
                for k in range(s_index, e_index):
                    elem_str += str(k) + symbol + token_list[k]
...
class ElementEvaluation(BaseEvaluation):
    ...
    def print_elem_result(self, input_ids, mask, write_path, drop_span=True):
        ...
        for index in range(len(input_ids)):
            ...
            write_str += self.elem_dict_to_string(token_list, self.predict_dict[index], symbol='&&') + "\n"
            write_str += self.elem_dict_to_string(token_list, self.gold_dict[index], symbol='&&') + "\n"
    ...
    def generate_elem_representation_new(self, gold_pair_label, bert_feature_embed, elem_feature_embed, feature_type=0, max_len_list=None):
        
        """
        used for transformers classifier

        :param gold_pair_label: [(s_index, e_index)] * 5
        :param bert_feature_embed: [N, sequence_length, hidden_dim] hidden_dim=768
        :param elem_feature_embed: [N, 4, sequence_lengh, feature_dim] feature_dim = 5
        ...
        for index in range(len(self.predict_dict)):
            ...
            for elem in self.elem_col:
                ...
                cur_candidate_pair_col = shared_utils.cartesian_product(cur_candidate_pair_col, cur_elem)
                # for candidate_pair in cur_candidate_pair_col:
                #     for index in range(len(candidate_pair)):
                #         s, e = candidate_pair[index]
                #         if s != -1:
                #             if s == e: e += 1
                #             max_len_list[index] = max(max_len_list[index], abs(e-s))
            candidate_pair_col.append(cur_candidate_pair_col)

        comp_input, token_type_input, padding_mask_input = self.create_pair_representation_new(
            candidate_pair_col, bert_feature_embed, elem_feature_embed, feature_type=feature_type, max_len_list=max_len_list 
        )
        ...
        return candidate_pair_col, comp_input, token_type_input, padding_mask_input, make_pair_label, elem_feature_embed, bert_feature_embed
    ...
    def create_pair_representation(self, candidate_col, feature_out, bert_feature_out, feature_type=0):
        ...
        if self.config.model_mode == "bert":
            encode_hidden_size = self.config.hidden_size
        else:
    ...
    def create_pair_representation_new(self, candidate_col, bert_feature_out, elem_feature_out, feature_type=0, max_len_list=None):
        """
        used for transformers classifier
        :param candidate_col: [n, tuple_pair_num, tuple_pair], tuple_pair: [(s_index, e_index)]
        ...
        :param max_len_list: 4-element list
        :param feature_type: 0 表示 5维 + 768维，1 表示 5维，2 表示 768维
        :return:
        comp_input: list of (list of (1 + subject_length + object_length + aspect_length + predicate_length) x feature_dim 2D lists)
        token_type_input: list of (list of lists)
        """
        comp_input, hidden_size = [], 5
        token_type_input = []
        padding_mask_input = []

        # sequence_length = bert_feature_out.size()[1]
        if self.config.model_mode == "bert":
            encode_hidden_size = self.config.hidden_size
        else:
            encode_hidden_size = self.config.hidden_size * 2
        ...
        for index in range(len(candidate_col)):
            ...
            for pair_index in range(len(candidate_col[index])):
                ...
                #     if s == -1:
                #         if self.config == "cuda":
                #             each_com_representation.append(torch.zeros(sequence_length).cpu().numpy().tolist())
                #         else:
                #             each_com_representation.append(torch.zeros(sequence_length).numpy().tolist())
                ...
                #         if self.config == "cuda":
                #             each_com_representation.append(xxx_tensor.cpu().numpy().tolist())
                #         else:
                #             each_com_representation.append(xxx_tensor.numpy().tolist())

                #         """
                #         each_com_representation: 4 x sequence_length 2D list
                #         """
                #         # 采用5维 + 768维
                ...
                for elem_index in range(4):
                    ...
                    if s != -1:
                        # 采用5维 + 768维
                        # elem_sub_feature_out = torch.cat(elem_feature_out[index][elem_index][s: e], dim=0).cpu().view(-1, hidden_size)
                        # bert_sub_feature_out = torch.cat(bert_feature_out[index][s: e], dim=0).cpu().view(-1, encode_hidden_size)
                        ...
                        if max_len_list is not None:
                            ...
                            if abs(e-s) < max_len_list[elem_index]:
                                each_padding_mask += [1] * abs(e-s) + [0] * (max_len_list[elem_index] - abs(e-s))
                            else:
                                each_padding_mask += [1] * max_len_list[elem_index]
                ...
                com_representation.append(cur_representation)
                token_type_representation.append(each_token_type_representation)
                padding_mask.append(each_padding_mask)
                """
                com_representation: list of (1 + subject_length + object_length + aspect_length + predicate_length) x feature_dim 2D lists
                token_type_representation: list of lists
                """

            # pair_representation = torch.stack(pair_representation, dim=0)

            comp_input.append(com_representation)
            token_type_input.append(token_type_representation)
            padding_mask_input.append(padding_mask)
            
        if max_len_list is not None:
            return comp_input, token_type_input, padding_mask_input
        else:
    ...
    def is_equal_tuple_pair(candidate_tuple_col, truth_tuple_col, null_pair):
        if isinstance(truth_tuple_col, np.ndarray):
            truth_tuple_col = truth_tuple_col.tolist()
        elif isinstance(truth_tuple_col, torch.TensorType):
            truth_tuple_col = truth_tuple_col.numpy().tolist()
...
class PairEvaluation(BaseEvaluation):
    ...
    def eval_model(self, measure_file, model=None, model_path=None, polarity=False, initialize=(False, False)):
        ...
        with open("/workspace/nlplab/kienvt/COQE/tmp/tuple_pair_output.txt", "w", encoding='utf-8') as f:
            f.write(tuple_str)
        ...
        if self.is_optimize(self.optimize_exact_measure, exact_measure, "pair"):
            ...
            if self.save_model and model_path is not None:
                torch.save(model, model_path)
